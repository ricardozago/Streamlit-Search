...
def pagination(total_pages: int, search: str, current_page: int, tags: str) -> str:
    ...
    hrefs = []
    if current_page != 1:
        hrefs.append(f'<a href="{params}&page={1}" target="_self">&lt&ltFirst </a>')
        hrefs.append(
    ...
    if current_page != total_pages:
        hrefs.append(
            f'<a href="{params}&page={current_page + 1}" target="_self">Next&gt</a>'
        )
...
def search_result(
    i: int, url: str, title: str, highlights: str, author: str, length: str, **kwargs
) -> str:
    ...
    return f"""
        ...
        <div style="font-size:95%;">
            <div style="color:grey;font-size:95%;">
                <a href="{url}" target="_self">{url[:90] + '...' if len(url) > 100 else url}</a>
            </div>
